...
import com.uptodd.uptoddapp.databinding.FragmentEssentialTodosBinding
...
class EssentialTodosFragment : Fragment(), TodosRecyclerAdapter.TodosInterface {

    private lateinit var binding: FragmentEssentialTodosBinding
    ...
    override fun onResume() {
        super.onResume()
        viewModel.loadEssentialsTodoScore()
    }
    ...
    override fun onCreateView(
        ...
        viewModel.essentialsPendingTodosList.observe(viewLifecycleOwner, {
            it?.let {
                if (it.size == 1) {
    ...
    private fun validateFreshness(): Boolean {
        val lastTodoFetchedOn =
            UptoddSharedPreferences.getInstance(requireContext()).getLastMonthlyAndEssentialsTodoFetchedDate()
...
//            return DateClass().isTodoFresh(lastTodoFetchedOn)
            ...
            return (daysBeforeLastTodoRefresh <= 30)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnDone.setOnClickListener {
            ...
            viewModel.markAllEssentialsAsComplete(
                todosRecyclerAdapter.selectedItemList,
                requireContext()
            )

            viewModel.multipleEssentialsSelectionTaskCompleteFlag.observe(viewLifecycleOwner, {
                if (it == true) {
                    todosRecyclerAdapter.clearAllSelected()
                    viewModel.multipleEssentialsSelectionTaskComplete()
                    viewModel.multipleEssentialsSelectionTaskCompleteFlag.removeObservers(
                        viewLifecycleOwner
                    )
                }
            })
    ...
    private fun initialiseBindingAndViewModel(inflater: LayoutInflater, container: ViewGroup?) {
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_essential_todos, container, false)
    ...
    private fun initialiseRecyclerView() {
        ...
        viewModel.essentialsPendingTodosList.observe(viewLifecycleOwner, { todosList ->
            todosList.let {
