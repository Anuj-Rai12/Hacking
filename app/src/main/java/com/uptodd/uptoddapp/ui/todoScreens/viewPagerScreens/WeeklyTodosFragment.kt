...
import com.uptodd.uptoddapp.databinding.FragmentWeeklyTodosBinding
...
class WeeklyTodosFragment : Fragment(), TodosRecyclerAdapter.TodosInterface {

    private lateinit var binding: FragmentWeeklyTodosBinding
    ...
    override fun onCreateView(
        ...
        viewModel.weeklyPendingTodosList.observe(viewLifecycleOwner, Observer {
            it?.let {
                if (it.size == 1) {
        ...
        viewModel.alarmSetToast.observe(viewLifecycleOwner, Observer {
            it?.let {
                if (it) {
                    Toast.makeText(
                        this.requireContext(),
        ...
        viewModel.alarmCancelledToast.observe(viewLifecycleOwner, Observer {
            it?.let {
                if (it) {
                    Toast.makeText(
                        this.requireContext(),
    ...
    private fun validateFreshness(): Boolean {
        val lastTodoFetchedOn =
            UptoddSharedPreferences.getInstance(requireContext()).getLastWeeklyTodoFetchedDate()
...
//            return DateClass().isTodoFresh(lastTodoFetchedOn)
            ...
            val daysBeforeLastTodoRefresh =
                DateClass().getDifferenceInDays(lastTodoFetchedOn, currentDate)
            return (daysBeforeLastTodoRefresh <= 7)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnDone.setOnClickListener {
            ...
            viewModel.markAllWeeklyAsComplete(
                todosRecyclerAdapter.selectedItemList,
                requireContext()
            )

            viewModel.multipleWeeklySelectionTaskCompleteFlag.observe(viewLifecycleOwner, Observer {
                if (it == true) {
                    todosRecyclerAdapter.clearAllSelected()
                    viewModel.multipleWeeklySelectionTaskComplete()
                    viewModel.multipleWeeklySelectionTaskCompleteFlag.removeObservers(
                        viewLifecycleOwner
                    )
                }
            })
    ...
    private fun initialiseBindingAndViewModel(inflater: LayoutInflater, container: ViewGroup?) {
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_weekly_todos, container, false)
    ...
    private fun initialiseRecyclerView() {
        ...
        viewModel.weeklyPendingTodosList.observe(viewLifecycleOwner, Observer { todosList ->
            todosList.let {
