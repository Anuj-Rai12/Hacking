...
class ViewTicket : Fragment() {
    ...
    private lateinit var binding: ViewTicketFragmentBinding
    ...
    override fun onCreateView(
        ...
        uptoddDialogs = UpToddDialogs(requireContext())
         ...
         binding=
            DataBindingUtil.inflate(inflater, R.layout.view_ticket_fragment, container, false)
        ...
        ToolbarUtils.initNCToolbar(
            requireActivity(), args.supportType, binding.toolbar,
            findNavController()
        )
    ...
    private fun initializeObservers(
        binding: ViewTicketFragmentBinding,
        adapter: TicketMessagesAdapter
    ) {
            ...
            0 -> {
                ...
                binding.ticketViewReopenTicket.setOnClickListener {
                    viewModel.reopenTicket(
                        "Reopen request on: ${AllUtil.getTimeFromMillis(System.currentTimeMillis())}",
                        ticket
                    )
                }
    ...
    override fun onDestroy() {
        super.onDestroy()
        val ticketViewModel = ViewModelProvider(this).get(AllTicketsViewModel::class.java)
